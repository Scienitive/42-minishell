...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/06/30 00:12:47 by alyasar           #+#    #+#             */
/*   Updated: 2022/07/03 18:51:04 by alyasar          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../minishell.h"

void	add_env_var(char *str, char **envp, t_gen *gen)
{
	char	*key;
	int		i;
	int		key_index;
	...
	key[i] = '\0';
	key_index = get_key_index(key, gen);
	free(key);
	if (key_index == -1)
		add_to_env(ft_strdup(str), gen);
...
void	export(char **args, char **envp, int fdout, t_gen *gen)
{
	...
	while (++i < get_string_arr_len(args))
	{
		if (is_key_valid(args[i]))
			add_env_var(args[i], envp, gen);
		else
			print_error("export: not a valid identifier.", gen, 1, 0);
	}
}
